...
import SpectraLoader from './SpectraLoader'
...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    return (
      ...
      >
        {loading && (
          // 20px is below MIN_FRAGMENT_SIZE_PX (48), so SpectraLoader
          // deliberately renders its plain CSS ring fallback here, not the
          // 9-fragment shield — matches the previous hand-rolled spinner's
          // `h-5 w-5` (20px) footprint.
          <span className="-ml-1 mr-3 inline-flex h-5 w-5 flex-shrink-0">
            <SpectraLoader variant="inline" size={20} label="Loading" />
          </span>
        )}
        {success && (
          <svg
